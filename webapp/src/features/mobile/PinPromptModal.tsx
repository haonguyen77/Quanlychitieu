...
/**
 * PinPromptModal — reusable PIN entry used for:
 *  - 'create': set a brand-new PIN (asks twice) — for first-launch / enabling encryption before sync.
 *  - 'enter':  enter an existing PIN — for unlocking / decrypting Drive data on a new device.
 *
 * The parent performs the actual crypto action in onSubmit(pin); this modal only
 * collects + validates input and surfaces an error string.
 */
interface Props {
  mode: 'create' | 'enter';
  title?: string;
  ...
  onCancel: () => void;
  onSkip?: () => void; // optional "Bỏ qua" (first-launch)
}
...
export function PinPromptModal({ mode, title, subtitle, busy, error, submitLabel, onSubmit, onCancel, onSkip }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-end sm:items-center justify-center" onClick={onCancel}>
      ...
      <div className="relative bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm p-5 space-y-4" onClick={e => e.stopPropagation()} style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 20px)' }}>
