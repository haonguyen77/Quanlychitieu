...
function getSmartDefault(data: FinanceData | null, moduleId: string, fieldName: string): string | null {
  ...
  // Return value from the most recently *created* record (createdAt = when user typed it in,
  // regardless of the transaction date field)
  ...
  // Fallback: most frequent value across all records
...
export function RecordFormDialog({ module, record, onClose, defaultAccount }: RecordFormDialogProps) {
  ...
  // ─── Apply smart defaults once data is available ─────────────────────────
  // Runs via useEffect (not lazy useState init) so it always sees the latest
  // store data — including the record the user just saved before opening this form.
