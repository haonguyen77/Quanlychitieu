...
function saveConfig(config: ReminderConfig) {
  ...
  // Tell background to reschedule — pass config since service worker can't access localStorage
  try { chrome.runtime?.sendMessage?.({ type: 'RESCHEDULE_REMINDERS', config }); } catch { /* */ }
}
