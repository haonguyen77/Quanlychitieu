...
export function PinLock() {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[var(--color-bg,#fff)] flex flex-col items-center justify-center px-8">
      <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-5" style={{ backgroundColor: '#6C2BD9' }}>
        <Lock size={28} color="white" />
      </div>
      <h2 className="text-lg font-bold text-[var(--color-text,#111)]">Nhập mã PIN</h2>
      <p className="text-sm text-[var(--color-text-secondary,#6b7280)] mt-1">Dữ liệu được mã hóa trên thiết bị này</p>
      ...
      <div className="grid grid-cols-3 gap-4">
        {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map(d => (
          <button key={d} onClick={() => press(d)} className="w-16 h-16 rounded-full text-2xl font-semibold text-[var(--color-text,#1f2937)] hover:bg-gray-100 active:bg-gray-200">{d}</button>
        ))}
        <div />
        <button onClick={() => press('0')} className="w-16 h-16 rounded-full text-2xl font-semibold text-[var(--color-text,#1f2937)] hover:bg-gray-100 active:bg-gray-200">0</button>
        <button onClick={back} className="w-16 h-16 rounded-full flex items-center justify-center hover:bg-gray-100 active:bg-gray-200"><Delete size={22} className="text-gray-500" /></button>
      </div>
