...
export function WineSettingsView({ onClearFilters: _onClearFilters }: Props) {
  ...
  const exportToGoogleSheets = async () => {
    ...
    try {
      // Use Chrome identity to get token
      const token = await new Promise<string>((resolve, reject) => {
        chrome.runtime?.sendMessage?.({ type: 'GET_AUTH_TOKEN', interactive: true }, (resp: any) => {
          if (resp?.token) resolve(resp.token); else reject(new Error(resp?.error || 'No token'));
        });
      });
      // Create spreadsheet
