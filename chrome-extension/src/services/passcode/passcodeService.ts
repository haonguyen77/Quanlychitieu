/**
 * PasscodeService — App-lock passcode for Chrome Extension.
 * COMPLETELY INDEPENDENT from encryption PIN.
...
class PasscodeService {
  ...
  isEnabled(): boolean { try { return localStorage.getItem(ENABLED_KEY) === '1'; } catch { return false; } }
  isUnlocked(): boolean { return !this.isEnabled() || this.unlocked; }
  isLocked(): boolean { return this.isEnabled() && !this.unlocked; }
  ...
  async setup(passcode: string): Promise<void> {
    ...
    localStorage.setItem(VERIFY_KEY, JSON.stringify({ salt: bufToB64(salt), iterations: ITERATIONS, hash }));
